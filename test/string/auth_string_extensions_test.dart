  ...
  group("Is Password Valid", () {
    ...
    group("Minimum Length", (){
      test("Password with less than 8 characters should be invalid", () {
        ...
        final isValid = password.isPasswordValid(password, minLength: 8);
        expect(isValid, false);
      });
    ...
    group("Minimum Uppercase", (){
      test("Password with less than 1 uppercase should be invalid", () {
        final password = "test1@";
        final isValid = password.isPasswordValid(password, minLength: 4, minUppercase: 1);
        expect(isValid, false);
      });

      test("Password with less than 3 uppercase should be invalid", () {
        final password = "testSD1@";
        final isValid = password.isPasswordValid(password, minLength: 4, minUppercase: 3);
        expect(isValid, false);
      });
    ...
    group("Minimum Number", (){
      ...
      test("Password with expected minimum numbers should be valid", () {
        ...
        final isValid = password.isPasswordValid(password, minLength: 4, minNumbers: 5, minUppercase: 1);
        expect(isValid, true);
      });
    });
    ...
    group("Special Characters", (){
      test("Password with no special character should be invalid", () {
        ...
        final isValid = password.isPasswordValid(password, minLength: 4, minUppercase: 1, minSpecialChars: 2);
        expect(isValid, false);
      });
    ...
    test("Password with all requirements should be valid", () {
      ...
      final isValid = password.isPasswordValid(password, minLength: 6, minNumbers: 3, minSpecialChars: 2, minUppercase: 4);
      expect(isValid, true);
    });
  });
}
